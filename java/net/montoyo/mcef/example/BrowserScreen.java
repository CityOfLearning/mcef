...
public class BrowserScreen extends GuiScreen {
	...
	public void drawScreen(int i1, int i2, float f) {
		...
		if (browser != null) {
			...
			browser.draw(.0d, height, width, 20.d); // Don't forget to flip Y
													// axis.
	...
	public void handleInput() {
		...
		while (Mouse.next()) {
			...
			if (browser != null) { // Inject events into browser. TODO: Handle
									// mods & leaving.
				int y = mc.displayHeight - sy - scaleY(20); // Don't forget to
															// flip Y axis.
	...
	public int scaleY(int y) {
		double sy = (((double) y) / ((double) height)) * (mc.displayHeight);
		return (int) sy;
	}
