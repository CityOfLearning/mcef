...
public class ScreenCfg extends GuiScreen {
	...
	public void handleInput() {
		...
		while (Mouse.next()) {
			...
			} else if (pressed && (btn == 0) && (sx >= x) && (sy >= y) && (sx < (x + width)) && (sy < (y + height))) { // In
				// browser
				// rect
				dragging = true;
				offsetX = x - sx;
				offsetY = y - sy;
			} else if (pressed && (btn == 0) && (sx >= (x + width)) && (sy >= (y + height)) && (sx < (x + width + 10))
					&& (sy < (y + height + 10))) {
				resizing = true;
			}
		}
	...
	public double unscaleX(int x) {
		return (((double) x) / ((double) mc.displayWidth)) * (super.width);
	}

	public double unscaleY(int y) {
		return (((double) y) / ((double) mc.displayHeight)) * (super.height);
	}
