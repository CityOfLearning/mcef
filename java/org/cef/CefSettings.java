...
public class CefSettings {
	...
	/**
	 * 32-bit ARGB color value, not premultiplied. The color components are
	 * always in a known order. Equivalent to the SkColor type.
	 */
	 ...
	 * sub-processes. By default the browser process executable is used. See the
	 * comments on CefExecuteProcess() for details. Also configurable using the
	 ...
	 * in-memory cache will be used for some features and a temporary disk cache
	 ...
	 * "persist-session-cookies" command-line switch.
	 */
	 ...
	 * default name of "debug.log" will be used and the file will be written to
	 * the application directory. Also configurable using the "log-file"
	 * command-line switch.
	 */
	 ...
	 * logged. Also configurable using the "log-severity" command-line switch
	 * with a value of "verbose", "info", "warning", "error", "error-report" or
	 * "disable".
	 */
	public LogSeverity log_severity = LogSeverity.LOGSEVERITY_DEFAULT;

	/**
	 * Custom flags that will be used when initializing the V8 JavaScript
	 * engine. The consequences of using custom flags may not be well tested.
	 * Also configurable using the "js-flags" command-line switch.
	 */
	 ...
	 * the module directory on Windows/Linux or the app bundle Resources
	 * directory on Mac OS X. Also configurable using the "resources-dir-path"
	 * command-line switch.
	 */
	 ...
	 * empty the locales directory must be located in the module directory. This
	 * value is ignored on Mac OS X where pack files are always loaded from the
	 * app bundle Resources directory. Also configurable using the
	 * "locales-dir-path" command-line switch.
	 */
	public String locales_dir_path = null;

	/**
	 * Set to true to disable loading of pack files for resources and locales. A
	 * resource bundle handler must be provided for the browser and render
	...
	/**
	 * By default CEF V8 references will be invalidated (the IsValid() method
	 * will return false) after the owning context has been released. This
	 * reduces the need for external record keeping and avoids crashes due to
	 * the use of V8 references after the associated context has been released.
	 *
	 * CEF currently offers two context safety implementations with different
	 * performance characteristics. The default implementation (value of 0) uses
	 * a map of hash values and should provide better performance in situations
	 * with a small number contexts. The alternate implementation (value of 1)
	 * uses a hidden value attached to each context and should provide better
	 * performance in situations with a large number of contexts.
	 *
