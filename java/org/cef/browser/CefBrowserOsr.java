...
public class CefBrowserOsr extends CefBrowser_N implements CefRenderHandler, IBrowser {
	...
	private Rectangle browser_rect_ = new Rectangle(0, 0, 1, 1); // Work around
	// CEF issue
	// #1437.
	...
	private CefBrowserOsr(CefClientHandler clientHandler, String url, boolean transparent, CefRequestContext context,
	...
	public void injectMouseButton(int x, int y, int mods, int btn, boolean pressed, int ccnt) {
		MouseEvent ev = new MouseEvent(dc_, pressed ? MouseEvent.MOUSE_PRESSED : MouseEvent.MOUSE_RELEASED, 0, mods, x,
				y, ccnt, false, btn);
		sendMouseEvent(ev);
	}
	...
	public void injectMouseMove(int x, int y, int mods, boolean left) {
		MouseEvent ev = new MouseEvent(dc_, left ? MouseEvent.MOUSE_EXITED : MouseEvent.MOUSE_MOVED, 0, mods, x, y, 0,
				false);
		sendMouseEvent(ev);
	}
	...
	public void injectMouseWheel(int x, int y, int mods, int amount, int rot) {
		MouseWheelEvent ev = new MouseWheelEvent(dc_, MouseEvent.MOUSE_WHEEL, 0, mods, x, y, 0, false,
				MouseWheelEvent.WHEEL_UNIT_SCROLL, amount, rot);
	...
	public void onPaint(CefBrowser browser, boolean popup, Rectangle[] dirtyRects, ByteBuffer buffer, int width,
		...
		synchronized (queue) {
			if (queue.size() > 16) { // Wayyyy to much; Minecraft must be
										// laggy...
