...
class CefBrowserWr extends CefBrowser_N {
	...
	private CefWindowHandlerAdapter win_handler_ = new CefWindowHandlerAdapter() {
		...
		public void onMouseEvent(CefBrowser browser, int event, int screenX, int screenY, int modifier, int button) {
			...
			if (event == MouseEvent.MOUSE_MOVED) {
				// Remove mouse-moved events if the position of the cursor
				// hasn't
				// changed.
				...
				// Change mouse-moved event to mouse-dragged event if the left
				// mouse
				// button is pressed.
			...
			// Send mouse event to the root UI component instead to the browser
			// UI.
			// Otherwise no mouse-entered and no mouse-exited events would be
			// fired.
			...
			if (event == MouseEvent.MOUSE_WHEEL) {
				...
				component_.dispatchEvent(new MouseWheelEvent(parent, event, now, modifier, pt.x, pt.y, 0, false,
						scrollType, 3, rotation));
			} else {
				clickCnt = getClickCount(event, button);
				component_.dispatchEvent(new MouseEvent(parent, event, now, modifier, pt.x, pt.y, screenX, screenY,
						clickCnt, false, button));
			}

			// Always fire a mouse-clicked event after a mouse-released event.
			if (event == MouseEvent.MOUSE_RELEASED) {
				component_.dispatchEvent(new MouseEvent(parent, MouseEvent.MOUSE_CLICKED, now, modifier, pt.x, pt.y,
						screenX, screenY, clickCnt, false, button));
			}
		}
	};
	...
	private CefBrowserWr(CefClientHandler clientHandler, String url, CefRequestContext context, CefBrowserWr parent,
		...
		// JComponent has clipping informations, which aren't accessible for
		// Canvas.
		component_ = new JPanel(new BorderLayout()) {
			@Override
			public void paint(Graphics g) {
				// If the user resizes the UI component, the new size and
				// clipping
				// informations are forwarded to the native code.
				// But on Mac the last resize information doesn't resize the
				// native UI
				// accurately (sometimes the native UI is too small). An easy
				// way to
				// solve this, is to send the last Update-Information again.
				// Therefore
				// we're setting up a delayedUpdate timer which is reset each
				// time
				// paint is called. This prevents the us of sending the UI
				// update too
				// often.
				doUpdate();
		...
		// On windows we have to use a Canvas because its a heavyweight
		// component
	...
	private void createUIIfRequired() {
		if ((getNativeRef("CefBrowser") == 0) && !isDisposed) {
			if (parent_ != null) {
				createDevTools(parent_, clientHandler_, getWindowHandle(), false, OS.isWindows() ? canvas_ : component_,
						inspectAt_);
			} else {
				createBrowser(clientHandler_, getWindowHandle(), url_, false, OS.isWindows() ? canvas_ : component_,
						context_);
			}
		}
	}
