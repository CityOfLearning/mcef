...
abstract class CefBrowser_N extends CefNativeAdapter implements CefBrowser {
	 ...
	 */
	protected void createBrowser(CefClientHandler clientHandler, long windowHandle, String url, boolean transparent,
			Component canvas, CefRequestContext context) {
	 ...
	 */
	protected final void createDevTools(CefBrowser parent, CefClientHandler clientHandler, long windowHandle,
			boolean transparent, Component canvas, Point inspectAt) {
		if ((getNativeRef("CefBrowser") == 0) && !isPending_) {
			try {
	 ...
	 * CefRenderHandler.startDragging call has ended either in a drop or by
	 * being cancelled. |x| and |y| are mouse coordinates relative to the
	 * upper-left corner of the view. If the web view is both the drag source
	 * and the drag target then all DragTarget* methods should be called before
	 * DragSource* methods. This method is only used when window rendering is
	 * disabled.
	 */
	 ...
	 * should not contain file contents as this type of data is not allowed to
	 * be dragged into the web view. File contents can be removed using
	 ...
	 * calling DragTargetDragEnter). This method is only used when window
	 * rendering is disabled.
	 */
	 ...
	 * DragTargetDragEnter call. This method is only used when window rendering
	 * is disabled.
	 */
	...
	private final native boolean N_CreateBrowser(CefClientHandler clientHandler, long windowHandle, String url,
			boolean transparent, Component canvas, CefRequestContext context);

	private final native boolean N_CreateDevTools(CefBrowser parent, CefClientHandler clientHandler, long windowHandle,
			boolean transparent, Component canvas, Point inspectAt);
	...
	private final native void N_Find(int identifier, String searchText, boolean forward, boolean matchCase,
			boolean findNext);
