...
public interface CefLifeSpanHandler {
	 ...
	 *
	 * If CEF created an OS window for the browser returning false will send an
	 * OS close notification to the browser window's top-level owner (e.g.
	 * WM_CLOSE on Windows, performClose: on OS-X and "delete_event" on Linux).
	 * If no OS window exists (window rendering disabled) returning false will
	 * cause the browser object to be destroyed immediately. Return true if the
	 * browser is parented to another window and that other window needs to
	 * receive close notification via some non-standard technique.
	 *
	 * @param browser
	 *            The browser generating the event.
	 * @return false will send an OS close notification to the browser window's
	 *         top-level owner.
	 */
	 ...
	 * execute any methods on the browser object after this callback returns. If
	 * this is a modal window and a custom modal loop implementation was
	 * provided in runModal() this callback should be used to exit the custom
	...
	/**
	 * Called on the IO thread before a new popup window is created. The
	 * |browser| and |frame| parameters represent the source of the popup
	 * request. The |target_url| and |target_frame_name| values may be empty if
	 * none were specified with the request. The |popupFeatures| structure
	 * contains information about the requested popup window. To allow creation
	 * of the popup window optionally modify |windowInfo|, |client|, |settings|
	 * and |no_javascript_access| and return false. To cancel creation of the
	 * popup window return true. The |client| and |settings| values will default
	 * to the source browser's values. The |no_javascript_access| value
	 ...
	 * begin running. Return false to use the default modal loop implementation
	 * or true to use a custom implementation.
	 *
	 * @param browser
	 *            The browser generating the event.
