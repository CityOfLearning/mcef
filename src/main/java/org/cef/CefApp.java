...
public class CefApp extends CefAppHandlerAdapter {
	...
	public enum CefAppState {
		/**
		 * No CefApp instance was created yet. Call getInstance() to create a
		 * new one.
		 */
		NONE,
		...
		/**
		 * CefApp is in its initializing process. Please wait until initializing
		 * is finished.
		 */
		INITIALIZING,
		 ...
		 * message loop is running. You can use all classes and methods of JCEF
		 * now.
		 */
		INITIALIZED,
		 ...
		 * instances will be disposed. No new CefClient or CefBrowser is allowed
		 * to be created. The message loop will be performed until all
		 * CefClients and all CefBrowsers are disposed completely.
		 */
		SHUTTING_DOWN,
	...
	/**
	 * According the singleton pattern, this attribute keeps one single object
	 * of this class.
	 */
	...
	private CefApp(String[] args, CefSettings settings) throws UnsatisfiedLinkError {
		...
		// on libcef so must be called first. This way we can store libs
		// elsewhere
		if (OS.isWindows()) {
		...
		// work because the dispatcher thread starts this shutdown hook (thread)
		// and
		// calls "join" on it. Therefore invokeAndWait will end up in a
		// deadlock.
		if (OS.isMacintosh()) {
			Runtime.getRuntime().addShutdownHook(new Thread("JCEF Shutdown Hook") {
				...
				public void run() {
					try {
						...
						// Avoid a deadlock. Give the native code at least 150
						// milliseconds
						// to terminate.
						Thread.sleep(150);
					} catch (Exception e) {
	...
	final protected void clientWasDisposed(CefClient client) {
		...
		if (clients_.isEmpty() && (getState().compareTo(CefAppState.SHUTTING_DOWN) >= 0)) {
			// Shutdown native system. This will shutdown the message loop as
			// well
			shutdown();
		}
	}
	 ...
	 * Calling this method closes all client instances with and all browser
	 * instances each client owns. After that the message loop is terminated and
	 * CEF is shutdown.
	 */
	public synchronized final void dispose() {
		...
		case SHUTTING_DOWN:
		case TERMINATED:
			// Ignore shutdown, CefApp is already terminated, in shutdown
			// progress
	...
	private final String getJcefLibPath() {
		...
		for (String path : paths) {
			...
			String[] found = dir.list((dir1, name) -> (name.equalsIgnoreCase("libjcef.dylib")
					|| name.equalsIgnoreCase("libjcef.so") || name.equalsIgnoreCase("jcef.dll")));
	 ...
	 * termination process is interrupted until CefApp calls
	 * continueTerminate().
	 */
...
//				                  "Chromium Embedded Framework.framework/Resources");
	 ...
	 * optional |domain_name|. An empty |domain_name| value for a standard
	 ...
	 * |scheme_name| is a custom scheme then also implement the
	 * CefApp::OnRegisterCustomSchemes() method in all processes. This function
	 * may be called multiple times to change or remove the factory that matches
	 * the specified |scheme_name| and optional |domain_name|. Returns false if
	 * an error occurs. This function may be called on any thread in the browser
	 * process.
	 */
