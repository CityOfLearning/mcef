 ...
 *
 * // Create and send a new query. var request_id = window.cefQuery({ request:
 * 'my_request', persistent: false, onSuccess: function(response) {}, onFailure:
 ...
 * function. B. The query is canceled in C++ code using the Callback::Failure
 * function. C. The context associated with the query is released due to browser
 ...
 * The Handler evaluates the request and returns the response. The query is then
 * discarded.
 *
 ...
 * available. The Handler cancels the subscription feed when there are no longer
 * any registered JavaScript receivers.
 *
 ...
 * browser or context instance passed into a single router callback must then be
 * passed into all router callbacks.
 *
 ...
 * browser basis.
 *
 * This implementation places no formatting restrictions on payload content. An
 * application may choose to exchange anything from simple formatted strings to
 * serialized XML or JSON data.
 *
 *
 * EXAMPLE USAGE
 *
 * 1. Define the router configuration. You can optionally specify settings like
 * the JavaScript function names. The configuration must be the same in both the
 * browser and renderer processes. If using multiple routers in the same
 * application make sure to specify unique function names for each router
 * configuration.
 *
 * // Example config object showing the default values. CefMessageRouterConfig
 * config = new CefMessageRouterConfig(); config.jsQueryFunction = "cefQuery";
 ...
 *
 * window.cefQuery({request: 'my_request', persistent: false, onSuccess:
 * function(response) { print(response); }, onFailure: function(error_code,
 * error_message) {} });
 *
 ...
 * boolean persistent, CefQueryCallback callback) { if
 * (request.indexOf("my_request") == 0) { callback.success("my_response");
 * return true; } return false; // Not handled. }
 *
...
public abstract class CefMessageRouter {
	...
	public static class CefMessageRouterConfig {
		/**
		 * Name of the JavaScript function that will be added to the 'window'
		 * object for sending a query. The default value is "cefQuery".
		 */
		public String jsQueryFunction;

		/**
		 * Name of the JavaScript function that will be added to the 'window'
		 * object for canceling a pending query. The default value is
		 * "cefQueryCancel".
		 */
	 ...
	 * the handler is added successfully or false if the handler has already
	 * been added. Must be called on the browser process UI thread. The Handler
	 * object must either outlive the router or be removed before deletion.
	 *
	 * @param handler
	 *            the according handler to be added
	 * @param first
	 *            if If set to true it will be added as the first handler
	 ...
	 *
	 * @param handler
	 *            the according handler to be removed
