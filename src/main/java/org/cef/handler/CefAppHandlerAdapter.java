...
/**
 * An abstract adapter class for managing app handler events. The methods in
 * this class are using a default implementation. This class exists as
	...
	public void onBeforeCommandLineProcessing(String process_type, CefCommandLine command_line) {
		if (process_type.isEmpty() && (args_ != null)) {
			...
			for (String arg : args_) {
				...
				// Arguments with '--', '-' and, on Windows, '/' prefixes are
				// considered switches.
				int switchCnt = arg.startsWith("--") ? 2 : arg.startsWith("/") ? 1 : arg.startsWith("-") ? 1 : 0;
				switch (switchCnt) {
				case 2:
					// An argument of "--" will terminate switch parsing with
