...
public abstract class CefCookieManager {
	 ...
	 * both will be deleted. If only |url| is specified all host cookies (but
	 * not domain cookies) irrespective of path will be deleted. If |url| is
	 * empty all cookies for all hosts and domains will be deleted. This method
	 * will be called on the IO thread. If posting the task was successful the
	 * method returns true.
	 */
	 ...
	 * will check for disallowed characters (e.g. the ';' character is
	 * disallowed within the cookie value attribute). This method will be called
	 * on the IO thread. If posting the task was successful the method returns
	 * true.
	 */
	 ...
	 * without an expiry date or validity interval) set |persistSessionCookies|
