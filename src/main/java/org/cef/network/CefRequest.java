...
public abstract class CefRequest {
	public static final class CefUrlRequestFlags {
		...
		/**
		 * If set upload progress events will be generated when a request has a
		 * body.
		 */
		public static final int UR_FLAG_REPORT_UPLOAD_PROGRESS = 1 << 3;

		/**
		 * If set the headers sent and received for the request will be
		 * recorded.
		 */
		public static final int UR_FLAG_REPORT_RAW_HEADERS = 1 << 5;

		/**
		 * If set the CefURLRequestClient.onDownloadData method will not be
		 * called.
		 */
		 ...
		 * of automatically re-tried. This currently only applies for requests
	...
	public enum ResourceType {
		...
		RT_SUB_RESOURCE, // !< Some other subresource. This is the default type
							// if
	...
	/**
	 * Transition qualifiers. Any of the core values above can be augmented by
	 * one or more qualifiers. These qualifiers further define the transition.
	 */
	...
	public enum TransitionType {
		/**
		 * Source is a link click or the JavaScript window.open function. This
		 * is also the default value for requests like sub-resource loads that
		 * are not navigations.
		 */
		TT_LINK(0),

		/**
		 * Source is some other "explicit" navigation action such as creating a
		 * new browser or using the LoadURL function. This is also the default
		 * value for navigations where the actual type is unknown.
		 */
		TT_EXPLICIT(1),

		/**
		 * Source is a subframe navigation. This is any content that is
		 * automatically loaded in a non-toplevel frame. For example, if a page
		 * consists of several frames containing ads, those ad URLs will have
		 * this transition type. The user may not even realize the content in
		 * these pages is a separate frame, so may not care about the URL.
		 */
		TT_AUTO_SUBFRAME(3),

		/**
		 * Source is a subframe navigation explicitly requested by the user that
		 * will generate new navigation entries in the back/forward list. These
		 * are probably more important than frames that were automatically
		 * loaded in the background because the user probably cares about the
		 * fact that this link was loaded.
		 */
		TT_MANUAL_SUBFRAME(4),

		/**
		 * Source is a form submission by the user. NOTE: In some situations
		 * submitting a form does not result in this transition type. This can
		 * happen if the form uses a script to submit the contents.
		 */
		TT_FORM_SUBMIT(7),

		/**
		 * Source is a "reload" of the page via the Reload function or by
		 * re-visiting the same URL. NOTE: This is distinct from the concept of
		 * whether a particular load uses "reload semantics" (i.e. bypasses
		 * cached data).
		 */
		TT_RELOAD(8);
		...
		/**
		 * Any of the core values above can be augmented by one or more
		 * qualifiers defined as TransitionFlags. These qualifiers further
		...
		/**
		 * Returns the integer representation of this enum, containing the
		 * source and the qualifier as one value.
		 *
	...
	/**
	 * Get the resource type for this request. Accurate resource type
	 * information may only be available in the browser process.
	 */
